...
function ChannelQQ() {
    ...
    this.sendBuddyMsg = function (args, callback) {
        ...
        var content = [content_str, ["font", {"name": "宋体", "size": size, "style": [0, 0, 0], "color": "000000"}]];
    ...
    /**
     * 发送临时消息
     * @param args [uin, id, content_str]
     * @param callback
     */
    this.sendSessMsg = function (args, callback) {
        var uin = args[0];
        var id = args[1];
        var content_str = args[2];
        msg_id++;
        var options = getDefaultOption('POST');
        options.path = '/channel/send_sess_msg2';
        var content = [content_str, ["font", {"name": "宋体", "size": 10, "style": [0, 0, 0], "color": "000000"}]];
        getSessGroupSig(uin, id, function (err, group_sig) {
            if (err) return   throwErr({'type': 'get_c2cmsg_sig2', 'msg': err});
            var r = {'to': uin,
                'group_sig': group_sig,
                'content': JSON.stringify(content),
                'msg_id': msg_id,
                'service_type': 0,
                'face': 0, 'clientid': clientid, 'psessionid': psessionid};
            var data = 'r=' + encodeURIComponent(JSON.stringify(r)) + '&clientid=' + clientid + '&psessionid=' + psessionid;
            pushMsg(options, 'sendSessMsg', data, callback);
        });
    ...
    this.sendqunmsg = function (args, callback) {
        ...
        var content = [msg, ["font", {"name": "宋体", "size": size, "style": [0, 0, 0], "color": "000000"}]];
    ...
    var checkMsgQueue = function () {
        ...
        nsutil.PostData(obj.option, obj.pdata, function (err, chk) {
            ...
            return obj.callback(err, ret);
        });
    };

    var getSessGroupSig = function (uin, id, callback) {
        var options = getDefaultOption('GET');
        var qo = {'clientid': clientid, 'psessionid': psessionid, 't': new Date().getTime()};
        qo['to_uin'] = uin;
        qo['id'] = id;
        qo['service_type'] = 0;
        var query = querystring.stringify(qo);
        options.path = '/channel/get_c2cmsg_sig2?' + query;
        throwLog({'type': 'get_c2cmsg_sig2', 'msg': options.path});
        nsutil.SendData(options, function (err, chunks) {
            throwLog({'type': 'get_c2cmsg_sig2_callback', 'msg': chunks});
            if (err) {
                throwErr({'type': 'get_c2cmsg_sig2', 'msg': err});
                return callback(err);
            }
            var ret = {};
            try {
            ...
            if (ret.retcode != 0) {
                return callback('获取临时会话签名错误');
            }
            return callback(err, ret.result.value);
        });
    };
