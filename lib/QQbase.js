...
QQBase.Encryption = function () {
    ...
    function getEncryption(password, salt, vcode, isMd5) {
        vcode = vcode || "";
        password = password || "";
        var md5Pwd = isMd5 ? password : md5(password), h1 = hexchar2bin(md5Pwd), s2 = md5(h1 + salt), rsaH1 = QQBase.RSA.rsa_encrypt(h1), rsaH1Len = (rsaH1.length / 2).toString(16), hexVcode = window.TEA.strToBytes(vcode.toUpperCase()), vcodeLen = "000" + vcode.length.toString(16);
    ...
    function getRSAEncryption(password, vcode, isMd5) {
        ...
        var str3 = QQBase.RSA.rsa_encrypt(str2);
        return str3
    }

    return {getEncryption: getEncryption, getRSAEncryption: getRSAEncryption, md5: md5,getEncryption:getEncryption}
}();


module.exports = QQBase.Encryption.getEncryption;